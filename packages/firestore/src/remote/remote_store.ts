...
export class RemoteStore {
  ...
  constructor(
    ...
    this.watchChangeAggregator = new WatchChangeAggregator(
      ...
      this.syncEngine.getSyncedDocuments
    );
  }
  ...
  private cleanUpWatchStreamState(): void {
    ...
    this.watchChangeAggregator.reset();
  }
  ...
  private async onWatchStreamChange(
    ...
    if (watchChange instanceof ExistenceFilterChange) {
      return this.handleExistenceFilter(watchChange, snapshotVersion);
    }
    ...
    this.watchChangeAggregator.add(watchChange);
    if (
      ...
      return this.handleWatchSnapshot(snapshotVersion);
    }
  }
   ...
   * SyncEngine.
   */
  private handleWatchSnapshot(snapshotVersion: SnapshotVersion): Promise<void> {
  ...
  private handleExistenceFilter(
    ...
    if (query.isDocumentQuery()) {
      if (expectedCount === 0) {
        ...
        this.watchChangeAggregator.addDocumentUpdate(deletedDoc);
      } else {
        assert(
          expectedCount === 1,
      ...
      if (currentSize !== expectedCount) {
        ...
        this.watchChangeAggregator.resetTarget(targetId);
